...
# [Day 01: Calorie Counting](https://adventofcode.com/2022/day/1 "Day 01")
...
```{r}
fn <- here("input","d01_input.txt")
calories <- read_lines(fn) %>%
    as.numeric() %>%
    tibble() %>%
    rename(.,calories=.) %>% 
...
calories_by_elf <- calories %>% 
    group_by(elf) %>% 
    summarise(n_calories = sum(calories,na.rm=TRUE))
...
```{r}
max(calories_by_elf$n_calories)
```
...
```{r}
calories_by_elf %>% 
    slice_max(n_calories,n = 3) %>%
    summarise(total = sum(n_calories))
```
